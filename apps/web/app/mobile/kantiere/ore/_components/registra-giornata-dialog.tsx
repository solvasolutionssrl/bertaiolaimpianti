...
/** Durata "umana" con unità: "45 min" / "1 h" / "1 h 30 min".
 *  Usata per il residuo (restano / di troppo) invece del formato H:MM. */
              ...
              <div className="space-y-2.5">
                ...
                {disponibili.length > 0 ? (
                  <button
                    ...
                    disabled={pending}
                    className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-dashed border-muted-foreground/30 bg-muted/30 py-2.5 text-sm font-medium text-muted-foreground hover:bg-muted/50 active:scale-[0.99] disabled:opacity-50"
                  >
              ...
              <div className="mb-3 space-y-2 rounded-xl border border-border bg-card p-3 shadow-[0_6px_18px_-5px_rgba(20,40,90,0.28)]">
                <div className="flex items-baseline justify-between gap-2">
                  ...
                  {righe.length === 0 || assegnato === 0 ? (
                    ...
                    <span className="text-xs font-semibold text-amber-600">Restano {fmtDurataUmana(restano)}</span>
                  ) : (
                    <span className="text-xs font-semibold text-amber-600">{fmtDurataUmana(-restano)} di troppo</span>
                  )}
                </div>
