...
| cloud_folder_path | text | es. `/Rossi_2026-05-10_SistemazioneBagno/` (provider storage TBD) |
| codice_interno | text | identificatore tecnico, es. `BER-26-001` |
| nome_cartella | text | nome leggibile; vedi §5 |
...
> 📌 **Specifica canonica**: `documentazione_generale/01_KICKOFF/Tassonomia_Lavori.md` §2.1–2.2. Quanto segue è la sintesi architetturale.

**Anagrafica e dati strutturati** (cliente, contatti, indirizzo, ticket, log tracciatura) vivono **esclusivamente nel DB Supabase** — mai duplicati in cartella. Le cartelle contengono solo file veri (PDF, foto, video, schemi).

**Nome cartella radice**: `<NomeCliente>_<YYYY-MM-DD>_<DescrizioneAI>`, dove `<DescrizioneAI>` è generato da Claude Haiku al momento della creazione (input: voci selezionate + note sopralluogo + indirizzo) e **editato dal capo** prima del conferma. Esempio: `Rossi_2026-05-10_SistemazioneBagno`.

**Sottocartelle** (nomi friendly, niente prefissi numerici):
```
<NomeCartellaCommessa>/
├── Preventivi/
├── Schemi/
├── Foto/
│   ├── Sopralluogo/
...
Le cartelle compaiono **istantaneamente** sui 5 PC ufficio via sync del provider storage scelto (TBD). Le voci di Sezione B selezionate dal capo possono aggiungere sottocartelle dedicate (es. `Foto/In corso/Solare/`), ma la struttura radice resta identica per tutte le commesse del tenant.
...
> **PWA = Progressive Web App**. Next.js servito con manifest + Service Worker. Tecnico apre il link `m.impiantixplus.app` da Safari iPhone → "Condividi" → "Aggiungi alla schermata Home" → icona impiantiXplus in home screen, esperienza full-screen come una app nativa. Niente App Store, niente Play Store, niente review.

1. Tecnico tocca l'icona impiantiXplus nella home iPhone → si apre PWA full-screen
...
7. Upload in background tramite **Service Worker** + Background Sync API:
   - File originale → storage cloud (path `/<NomeCartellaCommessa>/Foto/In corso/<voce>/<timestamp>.jpg`)
