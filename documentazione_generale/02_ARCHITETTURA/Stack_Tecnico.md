...
| **DNS** | Cloudflare (free) | CDN, DDoS, gestione CNAME multi-tenant + subdomain mobile (`m.<tenant>.impiantixplus.app`) |
...
- **Distribuzione frictionless**: WhatsApp un link → "Aggiungi alla Home" → in 30 secondi i tecnici hanno l'icona impiantiXplus sull'iPhone. Nessun App Store, nessun TestFlight, nessun account dev Apple/Google ($90+$25 risparmiati).
...
```
solva-impiantixplus/  (mono-repo Next.js)
├── apps/
│   ├── web/              # Next.js — host di tutte le superfici
│   │   ├── app/(office)/        # web ufficio (5 PC Bertaiola)
│   │   ├── app/(mobile)/        # PWA tecnici — route m.impiantixplus.app
