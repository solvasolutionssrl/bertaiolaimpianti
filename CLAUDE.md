...
- **Percorso**: partenza e rientro agli estremi, **chi guidava per ogni tratta con strada** (etichetta compatta, solo dove c'è strada), tratte fra cantieri costruite dal sistema (dirette con km/tempo, modificabili in «passando da una sede» / «passando da casa», tempo correggibile con motivo). Premendo «Registra giornata» il foglio **«Il viaggio»** chiede solo i dati obbligatori mancanti; la **barra dei tempi** (lavoro + viaggio, partenza/rientro) resta visibile sotto. Conferma passeggero **sempre**.
...
- **15/09/2026, prove da telefono**: partenza e rientro di default la sede predefinita, senza abitazione privata (tutto il giorno in sede = niente partenza e rientro); tempo correggibile su ogni tratta fra cantieri, con motivo; si indica solo l'ora di inizio (fine = inizio + ore + pausa + tratte, con il conto a vista), chi guidava per tratta, pausa pranzo arancione, card «La giornata» più bassa.
