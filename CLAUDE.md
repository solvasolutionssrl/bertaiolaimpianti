...
- **Tolta la vecchia «Ore su un cantiere, con viaggio»** (`manuale-dialog.tsx` e il suo banco): Registra giornata è l'unico inserimento a mano del tecnico. `registraOreManuali` resta sul server (non chiamata) e le tratte storiche che ha scritto contano ancora nel ricalcolo. Interfaccia di Registra giornata rimpicciolita del 10% circa su richiesta del cliente (a schermo era grandina).
