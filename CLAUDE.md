...
- **Sedi ↔ cantieri (regola ferrea)**: per un cantiere si propongono SOLO la **sede predefinita** (`sedi.is_default`, sempre) + le **sedi associate** a quel cantiere (`cantiere_sede`) + **"Abitazione privata"** a fine turno (0 km/0 tempo). MAI le sedi di altri cantieri. Applicata in tutte le UI (QR, app, capo, inserimento manuale ore che **filtra le sedi per cantiere scelto**) **e** ri-validata lato server da `sedeAmmessaPerCantiere` (`_actions/_lib/viaggio-timbra.ts`) in `validaViaggio(…, cantiereId)` e `registraOreManuali`. UI office **Sedi**: il collegamento cantieri usa un **dropdown di ricerca** (`sedi/_components/cantieri-collegati-select.tsx`), non l'intera lista di chip. → dettaglio in `documentazione_generale/08_LOGICHE/Logiche_Kantiere.md` (registro **regole/scelte operative** Kantiere: arrotondamenti, tolleranza, km, sedi, settings, auto-approvazione — **da tenere aggiornato**).
...
- **Ore in formato `H:MM`** (es. 7:30, non 7.5/2,6) in **tutte** le tabelle/KPI Kantiere (office desktop + mobile). ⚠️ **Eccezione: i TOTALI.** Su una somma `H:MM` diventa illeggibile (`705:19` per 65 giornate non dice niente e quei minuti non li usa nessuno): helper puro **`formattaOreTotale`** — sotto 1h → `45 min`, sotto 10h → `7:30`, sopra → `705 ore`. Usato nel banner soglia e nel KPI «Ore settimana». Il viaggio mostra sempre **km + tempo H:MM** (da `timbratura_viaggio.distanza_km`).
...
**Caso 4 — registra giornata SENZA timbrature**: chi non ha mai timbrato dichiara inizio/fine + pausa + cantieri/ore → azione **`registraGiornataDaZero`** (guardia **giornata VUOTA**, gate `registra_giornata_attivo`) = ingresso(inizio) + `calcolaSegmentiSplit`. UI `RegistraGiornataDialog` in tab Ore.

**Impostazioni ufficio "Turni & calcoli"** (`tenant_modules.config`, reader unico **`leggiImpostazioniTurno`**, tab in Impostazioni Kantiere): `tolleranza_chiusura_min` (5), `split_fine_turno_attivo` (on), `avvio_turno_libero` (on — **sostituisce il gate weekend**: se off i tecnici vedono solo i cantieri con QR attivo), `km_switch_attivo` (off, opt-in — gate del calcolo km nel cambio cantiere), `passo_minuti_stepper` (15, i +/- degli stepper), `registra_giornata_attivo` (on — gate caso 4). Per aggiungerne: campo nello schema di `salvaImpostazioniKantiere` + merge + reader + lettura nella page + UI.
